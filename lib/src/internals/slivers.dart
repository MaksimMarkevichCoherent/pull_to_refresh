...
class SliverRefresh extends SingleChildRenderObjectWidget {
  const SliverRefresh({
    ...
    Widget? child,
    this.refreshStyle,
  })  : assert(refreshIndicatorLayoutExtent >= 0.0, 'refreshIndicatorLayoutExtent must not be more or equal 0.0'),
...
class RenderSliverRefresh extends RenderSliverSingleBoxAdapter {
  RenderSliverRefresh(
      ...
      this.paintOffsetY,
      this.refreshStyle})
      : assert(refreshIndicatorExtent >= 0.0, 'refreshIndicatorExtent must not be more or equal 0.0'),
  ...
  set refreshIndicatorLayoutExtent(double value) {
    assert(value >= 0.0, 'value must not be more or equal 0.0');
  ...
  double get centerOffsetAdjustment {
    if (refreshStyle == RefreshStyle.Front) {
      ...
      return math.max(0, -renderViewport.offset.pixels);
    }
    return 0;
  }
...
class RenderSliverLoading extends RenderSliverSingleBoxAdapter {
  ...
  void performLayout() {
    ...
    assert(paintedChildSize.isFinite, 'paintedChildSize.isFinite must be true');
    assert(paintedChildSize >= 0.0, 'paintedChildSize must be more or equal 0.0');
    if (active) {
